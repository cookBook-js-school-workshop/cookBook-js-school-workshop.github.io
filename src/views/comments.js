...
export function commentsView(ctx, recipeId) {
    ...
    function update(active=false) {
        const result = commentsTemplate(loadComments(commentsPromise), ctx.user != null, active, onToggle, createSubmitHandler(onSubmit, 'content'))
    ...
    async function onSubmit({ content }) {
        ...
        result.owner = {usrname: ctx.user.username};
